...
<head>
    ...
    <title>Edit Profil Mahasiswa - {{ $mahasiswa->name ?? $mahasiswa->username }}</title>
    ...
    <main class="max-w-3xl mx-auto px-4 py-10 mt-20">
        <div class="bg-white p-8 rounded-lg shadow-lg">
            ...
            <div class="flex flex-col items-center mb-6"> {{-- Mengurangi mb-8 menjadi mb-6 --}}
                ...
                <label for="profile_picture_hidden_input" class="cursor-pointer group">
                    <div class="relative">
                        @if ($mahasiswa->profile_picture && Storage::disk('public')->exists($mahasiswa->profile_picture))
                            <img src="{{ asset('storage/' . $mahasiswa->profile_picture) }}" alt="Foto Profil Saat Ini" class="w-32 h-32 rounded-full mb-2 border-4 border-blue-200 object-cover group-hover:opacity-75 transition-opacity duration-300">
                        @else
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($mahasiswa->name ?? $mahasiswa->username) }}&background=random&color=fff&size=128" alt="Avatar Mahasiswa" class="w-32 h-32 rounded-full mb-2 border-4 border-blue-200 object-cover group-hover:opacity-75 transition-opacity duration-300">
                        @endif
                ...
                <h1 class="text-2xl font-bold text-gray-800 mt-4">{{ $mahasiswa->name ?? 'Nama Mahasiswa Belum Diatur' }}</h1>
                <p class="text-sm text-gray-500">{{ '@' . ($mahasiswa->username ?? 'N/A') }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('mahasiswa.profile.update') }}" enctype="multipart/form-data">
                ...
                <div class="space-y-4"> {{-- Mengurangi space-y-6 menjadi space-y-4 --}}
                    ...
                    <div>
                        ...
                        <input type="text" name="username" id="username" value="{{ old('username', $mahasiswa->username) }}" required
                ...
                <div class="mt-8 flex justify-end space-x-3">
                    <a href="{{ route('mahasiswa.profile') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium py-2 px-4 rounded-md">
                        Batal
                    </a>
