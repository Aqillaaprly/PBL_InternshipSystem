        ...
        <div class="bg-white p-8 rounded-xl shadow">
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex space-x-3">
                    <form method="GET" action="{{ route('admin.pendaftar.index') }}" class="flex space-x-2">
                    ...
                    <a href="{{ route('admin.pendaftar.create') }}" class="bg-blue-600 text-white px-5 py-2 rounded text-sm hover:bg-blue-700 whitespace-nowrap">+ Tambah Pendaftar</a>
                </div>
            </div>
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-50">
                                ...
                                <td class="px-5 py-3 text-center">
                                    <div class="flex item-center justify-center space-x-2">
                                        <a href="{{ route('admin.pendaftar.show', $pendaftar->id) }}" class="text-xs bg-sky-100 text-sky-600 hover:bg-sky-200 px-3 py-1 rounded">Detail</a>
                                        <a href="{{ route('admin.pendaftar.edit', $pendaftar->id) }}" class="text-xs bg-yellow-100 text-yellow-600 hover:bg-yellow-200 px-3 py-1 rounded">Ubah</a>
                                        <form action="{{ route('admin.pendaftar.destroy', $pendaftar->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pendaftar ini?');">
                                            @csrf
                                            @method('DELETE')
