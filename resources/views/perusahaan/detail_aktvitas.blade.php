...
<head>
    ...
    <title>Detail Aktivitas Mahasiswa: {{ $pendaftar->name ?? 'N/A' }}</title>    
...
<main class="pt-16 max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white p-8 rounded-xl shadow">
            <div class="pb-4 mb-4 border-b border-gray-200 flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 mb-2">Detail Kegiatan Magang Pendaftar</h1>
                    <p class="text-gray-600">Mahasiswa: **{{ $pendaftar->name ?? 'N/A' }}** (NIM: {{ $pendaftar->username ?? 'N/A' }})</p>
                    @php
                        ...
                        $pendaftarDiterima = $pendaftar->pendaftars->where('status_lamaran', 'Diterima')->first();
                ...
                <a href="{{ route('perusahaan.aktivitas-pendaftar.index') }}" class="text-blue-600 hover:underline text-sm">&larr; Kembali ke Daftar Mahasiswa</a>
            </div>
            ...
            @if ($aktivitas->isEmpty())
                <div class="bg-blue-100 border border-blue-400 text-blue-700 px-4 py-3 rounded relative" role="alert">
                    ...
                    <span class="block sm:inline">Pendaftar ini belum memiliki catatan aktivitas magang.</span>
                </div>
            @else
                                    ...
                                    <div class="modal-dialog modal-lg" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <p><strong>Mahasiswa:</strong> {{ $item->pendaftar->user->name ?? 'N/A' }} (NIM: {{ $item->pendaftar->user->username ?? 'N/A' }})</p>
                                ...
                                <div class="modal fade" id="verifyModal{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="verifyModalLabel{{ $item->id }}" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ route('perusahaan.aktivitas-pendaftar.verify', $item->id) }}" method="POST">
                                                @csrf
                                                <div class="modal-body">
