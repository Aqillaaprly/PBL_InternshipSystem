...
class CompanyController extends Controller
{
    ...
    {
        Log::info("AdminCompanyController@show: Attempting to find Company with ID: " . $companyId);
        $company = Company::find($companyId);

        if (!$company) {
            Log::error("AdminCompanyController@show: Perusahaan dengan ID {$companyId} tidak ditemukan.");
            abort(404, "Perusahaan dengan ID {$companyId} tidak ditemukan.");
        }
        $company->load('user');
